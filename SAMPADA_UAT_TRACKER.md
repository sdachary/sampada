...
| DEP-01 | Rails 7.2.3 is end-of-life (support ended 2026-08-09); upgrade to a supported Rails line | Dependencies | Medium | Verified |
...
- **Rails 8.1.3.1 upgrade applied (2026-09-06, commit `d76c1fd`):** Gemfile `rails '~> 8.1.3'` → resolved to **8.1.3.1** (Ruby 3.3.8, `.ruby-version` unchanged). `config.load_defaults` bumped 7.2 → **8.1** (absorbs 8.0+8.1 breaking changes in one jump). `db/schema.rb` regenerated under Rails 8.1 (alphabetically-sorted columns, `pg_catalog.plpgsql` extension, `ActiveRecord::Schema[8.1]`); no migration changes were needed. RuboCop autocorrected 61 new `Rails/StrongParametersExpect` offenses (`rubocop-rails` 2.35, `params[:id]` → `params.expect(:id)`). The `EOLRails` suppression (and EOLRuby) are **removed** from `ci.yml` — Brakeman `-A` now runs unsuppressed.
- **Forced verification tooling (oradb Docker):** upgrade exercised in an isolated Docker stack (ruby:3.3.8 + postgres:16 + redis:7) — full suite **388 examples / 0 failures** with functional redis (`rack-attack` active), RuboCop clean (201 files), Brakeman `-A --exit-on-warn` **0 warnings** (incl. EOLRails/EOLRuby pass), `bundle-audit` no vulnerabilities, production eager boot OK.
- **Brakeman ForceSSL finding (new with brakeman 8.0.6 `-A`):** flags `config.force_ssl` in `config/environments/production.rb` because it's assigned from `ENV.fetch('RAILS_FORCE_SSL', true)` (not a literal `true`), so the check cannot prove HTTPS is forced. This is **by design** — the app terminates TLS at Cloudflare Tunnel in front and toggles `force_ssl` via env, defaulting to `true`. Treated per SEC-04 mechanism: added to `config/brakeman.ignore` with justification (fingerprint `6a26086c…`), not blanket-suppressed.
- **rack-attack test-isolation fix:** `spec/rails_helper.rb` now resets `Rack::Attack.cache.store.clear` after request specs. Without it, the `api/write` throttle (30 writes/min/IP from `config/initializers/rack_attack.rb`) can trip in a single full-suite run when the store is a real Redis/MemoryStore and counters accumulate across examples → intermittent 429s. Pre-existing latent gap, not a Rails 8 regression; CI's shared Redis masked it by luck. Reset makes the suite deterministic.
- Files modified: `Gemfile`, `Gemfile.lock`, `config/application.rb`, `config/brakeman.ignore`, `db/schema.rb`, `spec/rails_helper.rb`, `app/controllers/api/*` (21 controllers, autocorrect), `.github/workflows/ci.yml`
...
| 2026-09-06 | DEP-01 (Rails 8.1.3.1 upgrade + EOLRails re-enabled) | CI on commit `d76c1fd`, run `34053912821` (GitHub Actions lint + frontend + test with Postgres 16 and Redis 7 services) | **Verified/Pass** — all 3 jobs green. `lint`: `bundle exec brakeman -A --no-pager --exit-on-warn` (suppression removed; EOLRails + EOLRuby pass), RuboCop clean, `bundle-audit` clean. `test`: **388 examples / 0 failures** on Rails 8.1.3.1 + `config.load_defaults 8.1`. `frontend`: lint + `npm audit` + build green. Upgrade (Gemfile `~> 8.1.3`, lockfile 8.1.3.1, schema regenerated, 61 rubocop autocorrects) pre-validated in an isolated ruby:3.3.8/postgres:16/redis:7 Docker stack on oradb. **Operator follow-up:** run one staging deploy + smoke test before promoting to production; rotate-secrets boot log will confirm AR-encryption boot under 8.1. |
