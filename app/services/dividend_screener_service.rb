class DividendScreenerService
  ...
  INDIAN_ETF_CANDIDATES = %w[NIFTYBEES.NS JUNIORBEES.NS].freeze
  ...
  def screen(target_income: nil, _risk_tolerance: 'moderate', market: 'IN')
    ...
    if target_income && screened.any?
      required_capital = begin
        target_income / (screened.first[:yield] / 100.0)
  ...
  def enrich(stock)
    ...
    return nil unless quote && quote[:price].to_f.positive?
