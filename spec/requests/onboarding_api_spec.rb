...
RSpec.describe 'Onboarding API', type: :request do
  ...
  describe 'GET /api/v1/onboarding/snapshot' do
    it 'returns zeroed snapshot when no data' do
      ...
      expect(json['checklist']).to eq({ 'loans' => false, 'investments' => false, 'insurance' => false, 'budget' => false })
      expect(json['onboarded']).to be(false)
    end

    it 'reflects existing data in snapshot and checklist' do
      create(:transaction, user: user, transaction_type: 'income', amount: 50000)
      create(:transaction, user: user, transaction_type: 'expense', amount: 12000)
      create(:debt, user: user, amount: 100000, emi_amount: 5000)
      create(:insurance_policy, user: user)
      get '/api/v1/onboarding/snapshot'
      json = JSON.parse(response.body)
      expect(json['money_in']).to eq(50000.0)
      expect(json['money_out']).to eq(12000.0)
      expect(json['total_owed']).to eq(100000.0)
  ...
  describe 'POST /api/v1/onboarding/complete' do
    it 'marks user onboarded' do
      ...
      expect(response).to have_http_status(:success)
      expect(JSON.parse(response.body)['onboarded']).to be(true)
